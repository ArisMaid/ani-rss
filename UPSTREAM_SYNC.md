...
本 fork 当前发布版本为 `3.2.28.63`（标签 `v3.2.28.63` 将指向本轮发布提交；上一版 `v3.2.28.62` 仍指向 `cce1190e`），上游前三段基线仍为 `v3.2.28`；本轮被测行为代码提交为 `f550cdddf57be2bf73185327e003aa2cbce18fbc`。本轮没有引入新的上游版本同步，所有差异均为 fork-local 修复与验收；后续同步只审阅当前基线标签到目标标签之间的上游提交，不重复引入更早历史。
