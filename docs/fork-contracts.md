...
| ID | 合同 | 主要证据 |
| --- | --- | --- |
| F43 | Mikan 评分异步补载返回后，当前星期展示顺序按最新有限非负评分降序更新；未知/缺失/非法评分置后，同分保持原顺序，不等待所有批次或重拉列表 | `MikanView.vue`、前端 35/35、功能修复隔离页面检查 |
| F44 | 首页非空当天订阅必须显式按需注册并渲染 `AniCoverView`；空列表保留空态，卡片冷加载不改变订阅数据来源 | `DashboardView.vue`、功能修复隔离页面检查 |
| F45 | 首页星期与停更天数依赖可更新的本地响应式时间；复用既有刷新、请求和可见恢复周期，不增加午夜定时器或后端日历服务 | `DashboardView.vue`、功能修复隔离日期检查 |
| F46 | 配置项“订阅排序”只描述订阅列表排序范围，不扩展为外部源通用排序设置 | `PageView.vue`、功能修复说明 |

F43–F46 不增加后端接口、排序服务、线程、数据库迁移或长期测试矩阵；实际下载器、账号和外部番剧源仍按验证文档标记为未连接。
