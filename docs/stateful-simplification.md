# 有状态模块依赖与本次决策（v3.2.28.67）

本文件按开发书 T7 建立依赖清单。`3.2.28.67` 在既有状态链合同上补齐 Mikan 异步评分展示顺序、首页非空订阅卡片注册和跨天本地日期响应式更新；真实数据库、文件系统和外部下载器成本仍明确后置。

## v3.2.28.67 新增状态验证

- Mikan 的 `data.weeks` 仍是补载和请求状态来源，展示层使用按星期复制的 `displayWeeks`；评分批次更新响应式 `score` 后自动重新排序，不改变 API 对象、不等待所有批次、不新增排序 worker。
- 首页当天订阅的数量和卡片使用同一 `todayAnis` 数据来源；`AniCoverView` 显式按需注册，空列表不触发卡片 chunk，非空列表的真实卡片加载不通过布局补丁伪造。
- 首页 `currentTime` 在既有请求/刷新/可见恢复周期更新，星期标签和停更天数共享本地日期依赖；不新增午夜定时器、时区配置或后端日历状态。
- 设置项语义限定为“订阅排序”，不把外部源排序扩展为新的全局状态链。
...
以下 SOP 只处理公共封面缓存维护暂停，不把普通源站暂时失败误判为缓存损坏，也不新增管理 API：

1. **触发条件：**持续出现 `public image cache maintenance is paused` 日志时再执行；普通源站暂时失败不需要重建缓存。
2. **理解影响：**当前进程会停止新增公共封面缓存，但仍可读取有效热缓存。这不等于订阅数据库或下载任务损坏，无须重装应用。
3. **定位路径：**以当前实例实际配置目录为根，公共缓存为 `<配置目录>/image-cache/public`，来源是 `ImageCacheService.publicRoot()`。容器部署必须根据挂载映射找到宿主机对应路径，不能照抄其他机器路径。
4. **先排原因：**检查磁盘空间、运行用户权限和文件占用；停止实例后再处理缓存文件。不要只反复重启，pending 超限记录仍在时启动会继续暂停。
5. **需要重建时：**确认目标仅为上述 `public` 子目录；管理员可以先备份到独立位置，再重建该专用封面缓存。缓存图片与 manifest 应一致处理；不要处理整个配置目录、`image-cache` 的其他用途目录、媒体或数据库，也不建议手工编辑计数来解除暂停。
6. **操作权限：**本代理不执行批量或递归删除，也不提供一键删除脚本；批量缓存重建由管理员手动完成。
7. **恢复确认：**处理原因后启动实例，确认告警不再持续，并打开一个此前未缓存的封面确认可以正常获取。无需核对每个历史图片，也不追求缓存零丢失。
8. **仍失败：**记录实际错误、部署方式和磁盘/权限情况，再针对原因排查；不要通过提高上限、关闭路径校验或恢复自动解除暂停来掩盖问题。
