...
本文件按修复计划 T7 建立依赖清单。T7 不是本次 3.2.28.59 发布的前置条件；没有固定下载器、文件树和数据库夹具时，不把推测性的删减写成性能结果。
...
- 下载器网络调用：`TorrentUtil` 的登录、任务列表、添加、删除、移动和重命名；任务列表读取已经移除固定 1 秒等待，失败不再伪装成空列表。
...
本次只收敛了下载器失败语义、刷新状态快照和无条件读取等待；没有删除状态枚举、数据库表、ownership/recovery/completion/backup 路径，也没有引入跨周期任务缓存。只有完成上述采样并补齐断电续跑测试后，才评估批量查询、周期内快照复用和完结迁移扫描收缩。
