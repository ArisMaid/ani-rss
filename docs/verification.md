...
本节覆盖 [`FORK_V3.2.32_SYNC_AND_SCROLL_OPTIMIZATION_PLAN.md`](../FORK_V3.2.32_SYNC_AND_SCROLL_OPTIMIZATION_PLAN.md) 的 U01–U08、S01–S04 实施；上游 `v3.2.32` 仅作局部对照，没有整体覆盖 fork。本轮精确 tag workflow 已完成，Release、后端门禁和镜像证据均按远端结果回填。
...
- bundle gate：Linux CI static `105,473/48,953 B`；login `203,162/63,229 B`；home `193,408/61,822 B`；subscriptions `192,403/65,174 B`（JS/CSS gzip），`forbiddenModules=[]`。
- 本机没有 Maven；[`build-test` run 35124869519](https://github.com/ArisMaid/ani-rss/actions/runs/35124869519) 与精确 [`tag build` run 35125384219](https://github.com/ArisMaid/ani-rss/actions/runs/35125384219) 均成功，Java 构建、SpotBugs/CI 门禁、打包、GitHub Release 和 GHCR 镜像以远端结果为准。

### 远端发布证据

- tag `v3.2.32.69` 精确指向 `38f26a8728f8bfad79d78f1e78089752a036118f`；[`GitHub Release v3.2.32.69`](https://github.com/ArisMaid/ani-rss/releases/tag/v3.2.32.69) 已发布且为非 draft、非 prerelease。
- Release 附件：`ani-rss.jar` SHA-256 `9e9d251f4516d2763874e223e949923fd7fddfc0e00f3db328d33e6f4c61f785`；`ani-rss.exe` SHA-256 `700cdfd5cbdd0474b9f48e62fcaa5abd077e71d2d23c951ab8537571ef99da4a`。
- GHCR manifest index：temurin `ghcr.io/arismaid/ani-rss:v3.2.32.69` / `sha256:da03c64570399d3140c3cc17bbaf74d8477827c2f932fe4cb46a73755b11b20d`（linux/amd64、linux/arm64）；openj9 `ghcr.io/arismaid/ani-rss:v3.2.32.69-openj9` / `sha256:403b2b21df88f4f386214b156ef7580ae89c54f6aaf098206ce802e4e0e2f2a7`（linux/amd64、linux/arm64）；arm32v7 `ghcr.io/arismaid/ani-rss:v3.2.32.69-arm32v7` / `sha256:37f24c3b98d2ba9e9181695c7c2719d0e811f96054eb93e0b05bde725e000e18`（linux/arm/v7）。Docker Hub 登录按 workflow 条件跳过（未配置凭据）；GHCR 三组镜像均已成功发布。
...
- tag `v3.2.28.68` 精确指向 `2587e9bcae147022c7a4b8d2c03c715f83e33bf3`；[`build-test` run 34815224498](https://github.com/ArisMaid/ani-rss/actions/runs/34815224498) 与正式 [`tag build` run 34815586170](https://github.com/ArisMaid/ani-rss/actions/runs/34815586170) 均成功。[GitHub Release v3.2.28.68](https://github.com/ArisMaid/ani-rss/releases/tag/v3.2.28.68) 已发布且为非 draft、非 prerelease。
- Release 附件：`ani-rss.jar` SHA-256 `8e57ac08b8ff843b8cb58f124f7d56a2eebfd47bd0175544f67a6cd312b3ae96`；`ani-rss.exe` SHA-256 `e2cf1ecc5a87637a16ca32e9f01d2b0e1e1c09ff1cd839122186e8e8986b92de`。
- GHCR 已核验 manifest index：temurin `ghcr.io/arismaid/ani-rss:v3.2.28.68` / `sha256:2ea6dcbec150464a178fc417bf44069de98e4a90a1e278d78ea2d70c5c86b2ff`（linux/amd64、linux/arm64）；openj9 `ghcr.io/arismaid/ani-rss:v3.2.28.68-openj9` / `sha256:369b31be9eaa498dbe781366f85498bccbb66122e188054d42f349e401c50aed`（linux/amd64、linux/arm64）；arm32v7 `ghcr.io/arismaid/ani-rss:v3.2.28.68-arm32v7` / `sha256:3cc683d09611227aa19fe77388be0c6bedeb74d10324b3432ca4faa793c21cbe`（linux/arm/v7）。Docker Hub 登录按 workflow 条件跳过（未配置 `DOCKER_USERNAME`/`DOCKER_PASSWORD`）；GHCR 三组镜像均已成功发布。
...
| 项目 | 状态 | 证据与边界 |
| --- | --- | --- |
| Mikan 异步评分后排序 | 通过 | `MikanView.vue` 的 `displayWeeks` 按最新有限评分降序重排，未知评分置后；前端 35/35 |
| 首页当天订阅卡片 | 通过 | `DashboardView.vue` 显式按需注册 `AniCoverView`，非空当天列表才加载；隔离 HTTP fixture 页面检查显示 2 张卡片 |
| 首页跨天日期/停更天数 | 通过 | `currentTime` 在既有请求、刷新和可见恢复周期更新；浏览器时钟跨天检查通过 |
| 设置项语义 | 通过 | `PageView.vue` 将“排序”明确为“订阅排序”，不扩展外部源通用排序配置 |

### 门禁与构建

- 前端 Vitest：5 个测试文件、35/35 通过；本轮未新增永久单元/回归测试。
- 生产 UI 构建：`pnpm exec vite build --outDir target/release-67-dist-20260909 --emptyOutDir false`，使用唯一输出目录且未清理既有输出；构建通过。
- bundle 门禁：static entry JS/CSS gzip 为 `105,227/48,953 B`；login `202,044/63,229 B`；home `192,264/61,822 B`；subscriptions `186,271/63,912 B`，均通过本轮明确预算；三个场景 `forbiddenModules=[]`。
- 本轮未修改 Java/后端；`.66` 的 Java 完整门禁证据继续保留在历史记录，正式发布工作流将重新执行项目构建。

### 人工检查与边界

- 生产页面检查使用隔离 HTTP fixture：非空当天订阅显示卡片，延迟评分返回后页面顺序变为最新分数降序，浏览器本地日期跨天后星期和空态更新。
- 本轮没有新增长期浏览器矩阵；卡片冷加载属于真实按需 chunk，包体门禁不把按需卡片误算为静态入口完整冷启动网络开销。
- 未连接真实下载器、账号、外部番剧源或生产媒体；不从 fixture 推导真实外部链路性能。

### 发布状态

- tag `v3.2.28.67` 精确指向 `7dfa07a5cce27d92330dc3562fc2b887b755e7d3`；[`build-test` run 34341684631](https://github.com/ArisMaid/ani-rss/actions/runs/34341684631) 与正式 [`tag build` run 34342179506](https://github.com/ArisMaid/ani-rss/actions/runs/34342179506) 均成功。[GitHub Release v3.2.28.67](https://github.com/ArisMaid/ani-rss/releases/tag/v3.2.28.67) 已发布且为非 draft、非 prerelease。
- Release 附件：`ani-rss.jar` SHA-256 `bf14c585b45ef6e1dc3eb4cf5e383fed9df0b19ba5b127df5ed11de3b8e8b646`；`ani-rss.exe` SHA-256 `4fb6b1c7d43ca21e6055b94e0b28d508d5a8e949ebcbed6a6a349b46d0d09548`。
- GHCR 已核验 manifest index：temurin `ghcr.io/arismaid/ani-rss:v3.2.28.67` / `sha256:d207f3ce31dbab4e7702d4dc6df55cd7c19bf4e9b72feb4702b85cf7f1d4e4fd`（linux/amd64、linux/arm64）；openj9 `ghcr.io/arismaid/ani-rss:v3.2.28.67-openj9` / `sha256:159b37df9b99bf6479f08f9918f76ffcfa992be2d3c1006f5c36219e2b34f2ef`（linux/amd64、linux/arm64）；arm32v7 `ghcr.io/arismaid/ani-rss:v3.2.28.67-arm32v7` / `sha256:2f09c2cb6c5fafaa723acded7b369fd4f301373f2e1ac76fb87da307838f4c8f`（linux/arm/v7）。Docker Hub 登录按 workflow 条件跳过（未配置 `DOCKER_USERNAME`/`DOCKER_PASSWORD`）；GHCR 三组镜像均已成功发布。
